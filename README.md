...
The app reads as one document, top to bottom, rather than a card floating on a page:

- **App bar** (top, navy) — the same three menu dropdowns as before (**Collection**: export/import the whole collection; **Templates**: template library, save as template, import template; **Checklist**: Rename, Duplicate, Delete checklist), plus a `Saved HH:MM` autosave stamp and the light/dark theme toggle. Only one dropdown is open at a time; click elsewhere or press Escape to close it.
- **Tab strip** — one tab per checklist, each showing a live `answered/total` completion count and, if locked, a small lock icon. The active tab reads as a raised card (a colored top edge plus side borders) rather than an underline.
- **The sheet** — a single ~800px reading column holding the collection name (a small eyebrow label above the title), the checklist title, the Inspector/Date fields, a completion progress bar, the item list, and one add-item row at the end. There's no separate header card for the collection — renaming it is still just clicking that eyebrow line and typing.
- **Action bar** (bottom, sticky) — an item-count readout on the left, then **Clear responses** (text), **Export PDF** (outline), and **Lock checklist** (filled) on the right — one instance of each, not duplicated between a header and footer like the previous layout.

Each checklist tab, and each item row, carries its own completion indicator: checking a box, filling in text, attaching a photo, or completing a sign-off marks that row (and the tab, and the header progress bar) as answered — section headers don't count toward the total. Each tab also gets an accent color, assigned by a deterministic hash of the checklist's id (so a tab keeps its color regardless of tab order or how many others are open) — purely a visual aid for telling tabs apart at a glance, not a status indicator. The palette draws one mid-tone step from each of the brand's Cypress Blue, Red, and Green swatches, picked so every color reads clearly against both the light and dark theme's background — mixing hues, rather than shading a single one, also makes neighboring tabs easier to tell apart than shades of blue alone would.

Each item row has a `⋮` menu (Move up, Move down, Duplicate, Delete — delete still needs a second click to confirm) instead of a separate drag handle and remove button. On a mouse (≥900px wide), hovering a row also reveals a drag handle next to the `⋮` for pointer-based reordering; on touch, press and hold a row for ~400ms to start dragging it instead.
...
- **Export…** — saves one template as a small `.checklist` file (native Save dialog on desktop, share sheet on iOS/Android, a folder-picker save dialog on the web where supported, otherwise a browser download) that can be emailed, AirDropped, or otherwise handed to someone else. On every platform the export lets you choose the destination folder rather than always landing wherever downloads go by default.
- **Export all templates…** — the same idea, bundled: every saved template in one `.checklist` file (a "pack"), for handing someone your whole set of checklists at once — the more common case in practice.
...
There's no server or account involved — sharing a template just means sharing the file. Each format is a small versioned JSON wrapper (`{ type, version, template }` for one, `{ type, version, templates }` for a pack) saved with a `.checklist` extension; importing validates the shape and rejects anything else with a plain-language error instead of crashing. On desktop, `.checklist` files carry the Cypressili mark as their icon in Finder/Explorer, and importing still accepts older `.json` exports too.
...
- **Export collection…** saves the whole collection — `collectionTitle`, `collectionDescription`, and every checklist with its items and their current responses — to one `.checklist` file, through the same native Save dialog / share sheet / folder-picker web download used everywhere else, letting you pick the destination folder.
...
Once a checklist's structure is finalized (or someone else is out in the field filling it in), "Lock checklist" in the action bar freezes its item list: no adding, removing, or reordering items — the `⋮` menu and drag handle disappear from every row (still taking up their layout space, so rows don't shift), and the add-item row dims and disables. Everything else about *using* the checklist still works while locked — checking boxes, typing responses, attaching photos, drawing signatures, even Clear responses — since the point is to protect the checklist's shape, not stop anyone from filling it in. A locked checklist shows a small lock icon on its tab so it's visible without opening any menu, and the Lock checklist button itself turns accent-red and relabels to "Unlock checklist" while engaged. The lock state is per-checklist (each tab locks independently) and travels with a checklist through collection export/import, but not through PDF export, since that's only ever a point-in-time snapshot.

Templates can be locked too — in the Template Library, next to Rename/Delete/etc. A template itself has no item-adding UI to protect (it's only ever edited by re-saving, renaming, or changing its description), so locking one does something more useful: every checklist made from it via "New checklist" starts pre-locked, so a standardized inspection template can guarantee its checklists arrive already protected instead of relying on whoever opens it to lock it themselves. A template's lock state travels with it through template/template-pack export and import, same as a checklist's does through collection files.
...
- The "Field Sheet" redesign's layout has no slot for the collection *description* (only the collection name, as a small eyebrow line above the checklist title) or the wordmark logo that used to sit in the collection header — the app bar's icon-only mark is the only logo now. `collectionDescription` is still read/written in the data model and still round-trips through collection export/import, it's just not editable from the UI anymore; ask if you'd like it (or the wordmark) restored somewhere.
- Per-item and per-signature timestamps aren't implemented — the item meta line reads `NN · TYPE` without a time, and a completed sign-off's card shows "Signed" without one either.
