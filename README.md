...
Everything in between — the collection title/description, checklist tabs, and the checklist itself — stays on the white panel background. Each tab gets an accent color from the brand's "Cypress Blue" scale (a deterministic hash of the checklist's id, so a tab keeps its color regardless of tab order or how many others are open) — purely a visual aid for telling tabs apart at a glance, not a status indicator. Light and dark theme use different slices of the same 9-step scale, since the palest steps wash out on the light background and the darkest steps do the same on the dark one.
